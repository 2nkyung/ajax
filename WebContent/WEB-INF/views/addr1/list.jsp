...
<select name="pageCount" onchange="changePageCount(this)">
   ...
   <table border="1">
      <tr>
         <th style="background-color: teal;">번호</th>
      ...
      <tr>
         ...
         <c:if test="${page!=1}">
            <a href="/addr/list?pageCount=${pageCount}&totalPageCnt=${totalPageCnt}">처음</a>
            <a href="/addr/list?page=${page-1}&pageCount=${pageCount}&totalPageCnt=${totalPageCnt}">-1</a>
         </c:if>   
         <c:if test="${page>10}">
            <a href="/addr/list?page=${page-10}&pageCount=${pageCount}&totalPageCnt=${totalPageCnt}">-10</a>
         </c:if>
            <c:forEach var="p" begin="${fBlock}" end="${lBlock}">
               <c:if test="${p!=page}">
                  <a href="/addr/list?page=${p}&pageCount=${pageCount}&totalPageCnt=${totalPageCnt}">[${p}]</a>
               </c:if>
               <c:if test="${p==page}">
                  <b>[${p}]</b>
               </c:if>
         ...
         <c:if test="${page!=totalPageCnt}">
            <a href="/addr/list?page=${totalPageCnt}&pageCount=${pageCount}&totalPageCnt=${totalPageCnt}">끝</a>
         </c:if>
         </td>
      </tr>
      <tr>
